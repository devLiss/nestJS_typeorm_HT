...
export class PairQuizGameRepository {
  ...
  async getCurrentGameInfo(gameId: string) {
    ...
    const query = `
      ...
      select qpr."questionId", qpr."answerStatus", to_char (qpr."addedAt"::timestamp with time zone at time zone 'Etc/UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as "addedAt" from quiz_progress qpr where "playerId" = qp."player1Id" order by qpr."addedAt" asc) t1) as "answers",
      ...
      (select row_to_json(x3)  from (select * from
      (select coalesce( array_to_json(array_agg( row_to_json(t1))),'[]') as answers from (
      select qpr."questionId", qpr."answerStatus",to_char (qpr."addedAt"::timestamp with time zone at time zone 'Etc/UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as "addedAt" from quiz_progress qpr where "playerId" = qp."player2Id" order by qpr."addedAt" asc) t1) as "answers",
